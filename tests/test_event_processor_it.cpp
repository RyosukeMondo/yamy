...
#include "../../src/core/engine/compiled_rule.h"
...
protected:
    ...
    void createRealSubstitutionTable() {
        ...
        auto add = [&](uint16_t in, uint16_t out) {
...
TEST_F(EventProcessorIntegrationTest, CompleteTransformation_R_to_E) {
    auto result = processor->processEvent(KEY_R, yamy::EventType::PRESS, modState.get());
...
TEST_F(EventProcessorIntegrationTest, CompleteTransformation_T_to_U) {
    auto result = processor->processEvent(KEY_T, yamy::EventType::PRESS, modState.get());
...
TEST_F(EventProcessorIntegrationTest, CompleteTransformation_D_to_Q) {
    auto result = processor->processEvent(KEY_D, yamy::EventType::PRESS, modState.get());
...
TEST_F(EventProcessorIntegrationTest, CompleteTransformation_0_to_R) {
    auto result = processor->processEvent(KEY_0, yamy::EventType::PRESS, modState.get());
...
TEST_F(EventProcessorIntegrationTest, ModifierSubstitution_PRESS_and_RELEASE) {
    ...
    auto press_result = processor->processEvent(KEY_N, yamy::EventType::PRESS, modState.get());
    EXPECT_EQ(press_result.output_evdev, KEY_LEFTSHIFT);
    EXPECT_EQ(press_result.type, yamy::EventType::PRESS);
    EXPECT_TRUE(press_result.valid);

    // Test N→LShift for RELEASE
    auto release_result = processor->processEvent(KEY_N, yamy::EventType::RELEASE, modState.get());
...
TEST_F(EventProcessorIntegrationTest, PreviouslyPartialKey_R_PRESS_and_RELEASE) {
    ...
    auto press_result = processor->processEvent(KEY_R, yamy::EventType::PRESS, modState.get());
    EXPECT_EQ(press_result.output_evdev, KEY_E);
    EXPECT_EQ(press_result.type, yamy::EventType::PRESS);
    EXPECT_TRUE(press_result.valid);

    // Test R→E for RELEASE (this worked before)
    auto release_result = processor->processEvent(KEY_R, yamy::EventType::RELEASE, modState.get());
...
TEST_F(EventProcessorIntegrationTest, PreviouslyPartialKey_T_PRESS_and_RELEASE) {
    ...
    auto press_result = processor->processEvent(KEY_T, yamy::EventType::PRESS, modState.get());
    EXPECT_EQ(press_result.output_evdev, KEY_U);
    EXPECT_EQ(press_result.type, yamy::EventType::PRESS);
    EXPECT_TRUE(press_result.valid);

    // Test T→U for RELEASE (this worked before)
    auto release_result = processor->processEvent(KEY_T, yamy::EventType::RELEASE, modState.get());
...
TEST_F(EventProcessorIntegrationTest, MultiplePassthroughKeys) {
    ...
    auto result_f = processor->processEvent(KEY_F, yamy::EventType::PRESS, modState.get());
    ...
    auto result_g = processor->processEvent(KEY_G, yamy::EventType::PRESS, modState.get());
    ...
    auto result_h = processor->processEvent(KEY_H, yamy::EventType::PRESS, modState.get());
...
TEST_F(EventProcessorIntegrationTest, CompletelyUnmappedEvdevCode) {
    ...
    auto result = processor->processEvent(999, yamy::EventType::PRESS, modState.get());
...
TEST_F(EventProcessorIntegrationTest, SpecialKeyTransformations) {
    // A→Tab
    auto result_a = processor->processEvent(KEY_A, yamy::EventType::PRESS, modState.get());
    EXPECT_EQ(result_a.output_evdev, KEY_TAB);
    EXPECT_EQ(result_a.output_yamy, 0x000F); // Tab scan code
    EXPECT_TRUE(result_a.valid);

    // B→Enter
    auto result_b = processor->processEvent(KEY_B, yamy::EventType::PRESS, modState.get());
...
TEST_F(EventProcessorIntegrationTest, NoRecursiveSubstitution) {
    ...
    auto result = processor->processEvent(KEY_E, yamy::EventType::PRESS, modState.get());
...
TEST_F(EventProcessorIntegrationTest, MultipleEventSequence) {
    ...
    auto w_press = processor->processEvent(KEY_W, yamy::EventType::PRESS, modState.get());
    ...
    auto w_release = processor->processEvent(KEY_W, yamy::EventType::RELEASE, modState.get());
    ...
    auto n_press = processor->processEvent(KEY_N, yamy::EventType::PRESS, modState.get());
    ...
    auto n_release = processor->processEvent(KEY_N, yamy::EventType::RELEASE, modState.get());
...
TEST_F(EventProcessorIntegrationTest, RepeatEventType) {
    ...
    auto result = processor->processEvent(KEY_W, yamy::EventType::REPEAT, modState.get());
