...
static bool sendAll(int sock, const void* buffer, size_t length) {
    ...
    while (remaining > 0) {
        ...
        if (sent <= 0) {
            ...
            LOG_DEBUG("[ipc] sendAll: send failed after {} bytes: {}",
                           ...
                           uintptr_t* result)
{
    ...
    if (sock == -1) {
        LOG_ERROR("[ipc] Failed to create socket: {}", std::strerror(errno));
        return false;
    }
    ...
    LOG_DEBUG("[ipc] sendCopyData: connecting to {}", addr.sun_path);
    ...
    if (connect(sock, (struct sockaddr*)&addr, sizeof(addr)) == -1) {
        ...
        LOG_DEBUG("[ipc] sendCopyData: connect failed: {}", std::strerror(errno));
        close(sock);
        return false;
    }

    LOG_DEBUG("[ipc] sendCopyData: connected, sending id={} size={}", data.id, data.size);
    ...
    if (!sendAll(sock, &data.id, sizeof(data.id))) {
        LOG_WARN("[ipc] sendCopyData: failed to send id");
        success = false;
    }

    // Send Size
    if (success && !sendAll(sock, &data.size, sizeof(data.size))) {
        LOG_WARN("[ipc] sendCopyData: failed to send size");
        success = false;
    }
    ...
    if (success && data.size > 0 && data.data) {
        if (!sendAll(sock, data.data, data.size)) {
            LOG_WARN("[ipc] sendCopyData: failed to send payload");
            success = false;
        }
    }
    ...
    if (success) {
        LOG_DEBUG("[ipc] sendCopyData: success (id={} size={})", data.id, data.size);
        ...
        LOG_WARN("[ipc] sendCopyData: failed");
    }
