...
public:
    ...
    bool operator()(const MR& i_what) {
        std::string id = i_what[1].str();
        std::string member = i_what[2].str();
        std::string value = i_what[3].str();

        // Convert to UTF-8
        id = to_utf8_compat(to_wstring(id));
        member = to_utf8_compat(to_wstring(member));
        value = to_utf8_compat(to_wstring(value));

        if (member == "path")
            (*m_directSSTPServers)[id].m_path = value;
        else if (member == "hwnd")
             (*m_directSSTPServers)[id].m_hwnd =
                reinterpret_cast<HWND>((LONG_PTR)_atoi64(value.c_str()));
        else if (member == "name")
...
void Command_DirectSSTP::exec(Engine *i_engine, FunctionParam *i_param) const
{
    ...
    for (std::list<std::string>::const_iterator
        ...
        if (_strnicmp("Charset", i->c_str(), 7) == 0 ||
                _strnicmp("Hwnd",    i->c_str(), 4) == 0)
            continue;
        if (_strnicmp("Sender", i->c_str(), 6) == 0)
            hasSender = true;
        request += i->c_str();
        request += "\r\n";
    }
    ...
    if (!hasSender) {
        request += "Sender: ";
        request += to_utf8_compat(loadString(IDS_mayu));
        request += "\r\n";
    }

    char buf[100];
    snprintf(buf, sizeof(buf)/sizeof(buf[0]), "HWnd: %Iu\r\n",
