...
- [x] 3. Fix EngineTestFixture initialization and synchronization
...
- [x] 7. Fix test isolation issue - segfault when running full test suite
  - File: tests/test_m00_integration.cpp (modify)
  ...
  - Individual tests pass (TapAShouldOutputB, HoldAPlusShouldOutputD work in isolation)
  - Crash occurs between tests during teardown/setup cycle
  - Root cause: Resource cleanup race condition between Engine threads and mock object deletion
  - Purpose: Achieve 100% test reliability by fixing test isolation
  - _Leverage: Engine::stop() thread synchronization, GoogleTest TearDown() lifecycle, pthread thread joining_
  - _Requirements: US-1 (Automated Integration Testing), US-2 (Root Cause Investigation)_
  - _Prompt: Role: Test Infrastructure Engineer | Task: Fix test isolation issue causing segfault when running full M00 integration test suite for spec m00-integration-test-automation. Current state: Individual tests pass (TapAShouldOutputB ✅ 2721ms, HoldAPlusShouldOutputD ✅ 2822ms when run alone), but segfault (exit 139) occurs after first test completes when running all 5 tests together. Root cause: MockInputHook's capturedKeyCallback is accessed by Engine threads during shutdown after TearDown() deletes the mock objects. Fix approaches: (1) Add explicit thread termination wait in TearDown() after engine->stop() (std::this_thread::sleep_for(100ms)), (2) Null out mockInputHook->capturedKeyCallback before deletion, (3) Verify Engine::stop() properly waits for all threads (check pthread_tryjoin_np logic), (4) Add proper synchronization barriers between TearDown() and next SetUp(). Test verification: Run full suite 10 times without crash | Restrictions: Do NOT disable tests - fix the root cause, do NOT add sleeps >200ms (maintain <10s total runtime), do NOT change test logic - only fix cleanup/synchronization, do NOT introduce new race conditions | Success: All 5 tests pass consecutively when run together, no segfaults in 10 consecutive full suite runs (./build_ninja/bin/yamy_m00_integration_test runs 10 times successfully), total execution time remains <10 seconds, individual tests still pass_
